...
struct yee_grid2d{
    ...
    eps(_m, _n),j_source(_m - 1, _n - 1), m_source(_m - 1, _n - 1), m(_m), n(_n)
    {
        assert(_m > 1);
        assert(_n > 1);
        e_x.setZero();
...
int main(){
    ...
    yee_grid2d<scalar> grid(256, 256);
    yee_grid2d<scalar> dual_grid(256, 256);
    ...
    for(size_t i = 0;i < grid.m - 1;i++){
        for(int j = -25;j < 25;j++){
            grid.h_z(i, j + 25) = std::exp(-j * j / 60.0);
        }
    }
    grid.sigma_e.fill(0);
    //grid.sigma_e.block(0, 70, grid.sigma_e.rows(), 3) = 1;
    Vector2f lenspoint1(0.50,0.60);
    Vector2f lenspoint2(0.50,0.90);
    ...
    for(size_t i = 0;i < grid.m - 2;i++){
        for(size_t j = 0;j < grid.n - 2;j++){
            Vector2f vec(float(i) / grid.m, float(j) / grid.n);
    ...
    sf::Texture tex;
    double time = 0;
    tex.create(grid.m - 1, grid.n - 1);
    sf::Sprite schprit(tex);
    schprit.setScale(window.getSize().x / (grid.m - 1.0),window.getSize().y / (grid.n - 1.0));
    ...
    while (window.isOpen()){
        ...
        for(size_t i = 0;i < 1;i++){
            const decltype(grid)::scalar tstp = 0.0005 / c;
            ...
            vector3 H_at_point(0, 0, grid.h_z(grid.m / 2, 7 * grid.n / 8));
            vector3 gE_at_point(0,0,0);
            vector3 gH_at_point(0, 0, dual_grid.h_z(dual_grid.m / 2, 7 * dual_grid.n / 8));
            vector6 gEandH;
        ...
        //std::terminate();
        std::vector<unsigned char> texdata((grid.m - 1) * (grid.n - 1) * 4);
        elong = 
          ((grid.e_x.block(1, 0, grid.m - 1, grid.n - 1) + grid.e_x.block(0, 0, grid.m - 1, grid.n - 1)) * 0.5).square() 
        + ((grid.e_y.block(0, 1, grid.m - 1, grid.n - 1) + grid.e_y.block(0, 0, grid.m - 1, grid.n - 1)) * 0.5).square()
        + (grid.h_z).square();
        ...
        for(size_t i = 0;i < grid.m - 1;i++){
            for(size_t j = 0;j < grid.n - 1;j++){
                if(i == grid.m / 2 && j == 7 * grid.n / 8){
                    texdata[(i * (grid.n - 1) + j) * 4] =     255;
                    texdata[(i * (grid.n - 1) + j) * 4 + 1] = 0;
                    texdata[(i * (grid.n - 1) + j) * 4 + 2] = 0;
                    texdata[(i * (grid.n - 1) + j) * 4 + 3] = 255;
                }
                else{
                    texdata[(i * (grid.n - 1) + j) * 4] = fcolor(std::abs(grid.h_z(i,j)));
                    texdata[(i * (grid.n - 1) + j) * 4 + 1] = fcolor(relu(dual_grid.h_z(i,j)));//fcolor(std::abs(grid.e_x(i,j)));
                    texdata[(i * (grid.n - 1) + j) * 4 + 2] = fcolor(grid.mu(i, j) / mu_0 / 3);//fcolor(std::abs(grid.e_y(i,j)));;
                    texdata[(i * (grid.n - 1) + j) * 4 + 3] = 255;
                }
            }
        }
        ...
        sf::Text info_grad("Yellow (actually only green) = positive magnetic gradient", font);
